% 14-equation Smets & Wouters (07) model. 
% line 202: 'original' replicates SW07 exactly; 'updated' uses extended sample

// 1. Declare model variables
var ewma, epinfma, zcapf, rkf, kf, pkf, cf, invef, yf, labf, wf, rrf, mc, zcap, rk, k, pk, c, inve, y, lab, pinf, w, r, a, b, g, qs, ms, spinf, sw, kpf, kp, // model variables    
    labobs robs pinfobs dy dc dinve dw; // observation variables
varexo ea eb eg  eqs  em  epinf ew;  // structural shocks
...
var eb; // risk premium
stderr 1.8513;
var eg; // government spending
stderr 0.6090;
var eqs; // investment-specific technology
stderr 0.6017;
var em; // monetary policy
stderr 0.2397;
var epinf; // price markup
stderr 0.1455;
var ew; // wage markup
stderr 0.2089;
end;
...
constebeta,0.16,0.01,2.0,GAMMA_PDF,0.25,0.1;
constelab,0,-10.0,10.0,NORMAL_PDF,0.0,2.0;
...
// 6. Estimation
// original: from AER publication
estimation(optim=('MaxIter',200),datafile=data_for_dynare_original,mode_compute=4,presample=4,lik_init=2,mh_replic=50000,mh_nblocks=2,mh_jscale=0.35,mh_drop=0.2);
// updated data series
//estimation(datafile=data_for_dynare,mode_compute=4,lik_init=2,mh_replic=50000,mh_nblocks=2,mh_jscale=0.20,mh_drop=0.2);

// 7. Evaluate results
shock_decomposition dy pinfobs robs;
stoch_simul(irf=16) y pinf r ;